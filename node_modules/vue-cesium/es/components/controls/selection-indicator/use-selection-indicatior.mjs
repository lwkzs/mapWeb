...
import { pickImageryLayerFeatures } from './util.mjs';
...
function useSelectionIndicatior(instance, props, $services) {
  ...
  const computeScreenSpacePosition = (position2, result) => {
    const { viewer } = $services;
    return compareCesiumVersion(Cesium.VERSION, "1.121") ? Cesium.SceneTransforms.worldToWindowCoordinates(viewer.scene, position2, result) : Cesium.SceneTransforms["wgs84ToWindowCoordinates"](viewer.scene, position2, result);
  };
  const update = () => {
