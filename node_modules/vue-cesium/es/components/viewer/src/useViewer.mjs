...
import { isEmptyObj, dirname } from '../../../utils/util.mjs';
import { getInstanceListener, $ } from '../../../utils/private/vm.mjs';
import { compareCesiumVersion, setViewerCamera } from '../../../utils/cesium-helpers.mjs';
...
import useVcExtension from '../../../composables/private/use-vc-extension.mjs';
...
function useViewer(props, ctx, vcInstance) {
  ...
  const load = async function() {
    ...
    if (compareCesiumVersion(Cesium.VERSION, "1.104")) {
      options.baseLayer = isEmptyObj(options.baseLayer) && options.baseLayer !== false ? ImageryLayer.fromProviderAsync(TileMapServiceImageryProvider.fromUrl(url), {}) : options.baseLayer;
    } else {
  ...
  const getCesiumScript = async function() {
    ...
    if (!globalThis.Cesium) {
      ...
      if (mars3dConfig) {
        ...
        const dirName2 = dirname(dcConfig.Cesium);
