...
import { makeCartesian3Array, getFirstIntersection, getGeodesicDistance, calculateAreaByPostions, compareCesiumVersion } from '../../utils/cesium-helpers.mjs';
import { getCurrentInstance, ref, computed, reactive, nextTick, h } from 'vue';
      ...
      if (cmpName === "VcAnalysisSightline") {
        ...
        for (let i = 0; i < positions.length - 1; i++) {
          ...
          if (s > 0 && positions.length > 2 && props.showDistanceLabel) {
            labels.push({
              text: distanceFormatter(s, (_b = props.measureUnits) == null ? void 0 : _b.distanceUnits, props.locale, (_c = props.decimals) == null ? void 0 : _c.distance),
          ...
          if (positions.length > 2 && props.showAngleLabel) {
            if (i > 0 || props.loop) {
              ...
              angles.push(angle);
              labels.push({
                text: angleFormatter(angle, (_d = props.measureUnits) == null ? void 0 : _d.angleUnits, props.locale, (_e = props.decimals) == null ? void 0 : _e.angle),
                position: point1,
                id: createGuid(),
                ...polylineLabelsOpts
              });
        ...
        if (props.showLabel && positions.length) {
          if (cmpName.includes("Area")) {
            labels.push({
              text: areaFormatter(area, (_f = props.measureUnits) == null ? void 0 : _f.areaUnits, props.locale, (_g = props.decimals) == null ? void 0 : _g.area),
            ...
            labels.push({
              text: distanceFormatter(distance, (_h = props.measureUnits) == null ? void 0 : _h.distanceUnits, props.locale, (_i = props.decimals) == null ? void 0 : _i.distance),
              position: positions[positions.length - 1],
              id: createGuid(),
              ...polylineLabelOpts
            });
  ...
  const updateLabelPosition = () => {
    computedRenderDatas.value.forEach((polyline, index) => {
      ...
      if (!(positions.length < 2)) {
        ...
        const positionWindow = compareCesiumVersion(Cesium.VERSION, "1.121") ? SceneTransforms.worldToWindowCoordinates(scene, startPosition, {}) : SceneTransforms["wgs84ToWindowCoordinates"](scene, startPosition, {});
        ...
        for (let i = 1; i < positions.length; i++) {
          const positionWindow2 = compareCesiumVersion(Cesium.VERSION, "1.121") ? SceneTransforms.worldToWindowCoordinates(scene, positions[i], {}) : SceneTransforms["wgs84ToWindowCoordinates"](scene, positions[i], {});
          if (defined(positionWindow2)) {
            ...
            if (labels[i - 1] !== labelTotalLength) {
              if (defined((_b = labels[i - 1]) == null ? void 0 : _b.horizontalOrigin)) {
