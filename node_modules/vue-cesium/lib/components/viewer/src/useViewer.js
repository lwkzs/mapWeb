...
var useVcExtension = require('../../../composables/private/use-vc-extension.js');
...
function useViewer(props, ctx, vcInstance) {
  ...
  const { invokeExtensions, revokeExtensions } = useVcExtension["default"]();
  vue.watch(
    ...
    async (val) => {
      ...
      if (defined(viewer.baseLayerPicker) && !viewer.baseLayerPicker.isDestroyed() && !val) {
        ...
        const baseLayer = viewer.imageryLayers.addImageryProvider(
          cesiumHelpers.compareCesiumVersion(Cesium.VERSION, "1.104") ? await Cesium.TileMapServiceImageryProvider.fromUrl(url) : new Cesium.TileMapServiceImageryProvider({
  ...
  const load = async function() {
    ...
    options.fullscreenElement = util.isEmptyObj(options.fullscreenElement) ? vm.$(viewerRef) : options.fullscreenElement;
    if (cesiumHelpers.compareCesiumVersion(Cesium.VERSION, "1.104")) {
      options.baseLayer = util.isEmptyObj(options.baseLayer) && options.baseLayer !== false ? ImageryLayer.fromProviderAsync(TileMapServiceImageryProvider.fromUrl(url), {}) : options.baseLayer;
    } else {
      options.imageryProvider = util.isEmptyObj(options.imageryProvider) ? new TileMapServiceImageryProvider({ url }) : options.imageryProvider;
    }
    let viewer;
    if (props.viewerCreator) {
      viewer = props.viewerCreator(vcInstance, vm.$(viewerRef), options);
    } else {
      if (globalThis.mars3d) {
        vcInstance.map = new mars3d.Map(vm.$(viewerRef).id, {
          scene: options,
          control: options
        });
    ...
    if (cesiumHelpers.compareCesiumVersion(Cesium.VERSION, "1.83")) {
      if (cesiumHelpers.compareCesiumVersion(Cesium.VERSION, "1.116")) {
  ...
  const getCesiumScript = async function() {
    ...
    if (!globalThis.Cesium) {
      ...
      if (mars3dConfig) {
        const libsConfig = mars3dConfig.libs || loadUtil.getMars3dConfig();
        ...
        const dirName2 = util.dirname(dcConfig.Cesium);
        loadLibs.push(`${dirName2}/Widgets/widgets.css`);
        if (dcConfig.turf) {
      ...
      return Promise.all(scriptLoadPromises).then(() => {
        if (globalThis.Cesium) {
          ...
          if (cesiumHelpers.compareCesiumVersion(globalThis.DC.VERSION, "3.0.0")) {
