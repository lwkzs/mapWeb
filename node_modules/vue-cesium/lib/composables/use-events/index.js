  ...
  function pickedAction(movement) {
    ...
    pickedFeatureAndCallbackNames.forEach((item) => {
      ...
      if (pickedFeature2.id) {
        if (shared.isArray(pickedFeature2.id)) {
    ...
    eventSourceList.forEach((event) => {
      if (event.callbackName) {
        const fn = event.cesiumObject[`vc${event.callbackName}`] || event.cesiumObject[`on${shared.capitalize(event.callbackName)}`] || event.cesiumObject[util.kebabCase(`on${shared.capitalize(event.callbackName)}`)];
