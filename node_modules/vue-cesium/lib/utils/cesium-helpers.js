...
require('../shared/extends/materials/index.js');
...
function makePolygonHierarchy(val, ellipsoid, isConstant = false) {
  ...
  if (shared.isPlainObject(val) && shared.hasOwn(val, "positions")) {
    const value = lodash.cloneDeep(val);
...
function makeMaterialProperty(val, isConstant = false) {
  ...
  if (val instanceof CallbackProperty || val instanceof Color || val instanceof CheckerboardMaterialProperty || val instanceof ColorMaterialProperty || val instanceof CompositeMaterialProperty || val instanceof GridMaterialProperty || val instanceof ImageMaterialProperty || val instanceof MaterialProperty || val instanceof PolylineArrowMaterialProperty || val instanceof PolylineDashMaterialProperty || val instanceof PolylineGlowMaterialProperty || val instanceof PolylineOutlineMaterialProperty || val instanceof StripeMaterialProperty || val instanceof VcCircleWaveMaterialProperty["default"] || val instanceof VcLineFlowMaterialProperty["default"] || val instanceof VcLineTrailMaterialProperty["default"]) {
    return val;
  }
  ...
  if (shared.isPlainObject(val) && shared.hasOwn(val, "fabric")) {
    ...
    switch (value.fabric.type) {
      ...
      case "VcCircleWave": {
        return new VcCircleWaveMaterialProperty["default"]({
      ...
      case "VcLineFlow": {
        return new VcLineFlowMaterialProperty["default"]({
      ...
      case "VcLineTrail": {
        return new VcLineTrailMaterialProperty["default"]({
          image: defaultValue(value.fabric.uniforms.image, Material.DefaultImageId),
...
function compareCesiumVersion(a, b, operator = ">=") {
  a = a || "1.6.7";
  return compareVersions.compare(a, b, operator);
}
...
exports.compareCesiumVersion = compareCesiumVersion;
...
exports.makeImageBasedLighting = makeImageBasedLighting;
